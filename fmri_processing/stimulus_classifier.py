...
class StimulusClassifier:
    ...
    def train_best_model_by_recall_(self, X, y, target_class=1, random_state=42, verbose=False): 
    ...
    def train_best_model_by_recall_(self, train_matrix, y, target_class=1, random_state=42, verbose=True):
        X = train_matrix
        labels = y
        groups = np.repeat(np.arange(X.shape[0] // 5), 5)  # Группы по 5 стимулов

        # 1. Убираем ADASYN из пайплайна и заменяем на class_weight
        def create_pipeline(model):
            return Pipeline([  # Используем стандартный Pipeline вместо ImbPipeline
                ('scaler', StandardScaler()),
                ('feature_selector', 'passthrough'),
                ('model', model)
            ])

        # Рассчитываем соотношение классов для XGBoost
        n_positive = sum(labels)
        n_negative = len(labels) - n_positive
        ratio = n_negative / n_positive if n_positive > 0 else 1

        # 2. Обновленные модели с правильными параметрами балансировки
        models = {
            "Logistic Regression": {
                'pipeline': create_pipeline(LogisticRegression(max_iter=1000)),
                'params': {
                    'feature_selector': [
                        PCA(n_components=0.95),  # Убираем проблемные селекторы
                        'passthrough'  # Добавляем опцию без селектора
                    ],
                    'model__C': [0.1, 1, 10, 100],
                    'model__class_weight': ['balanced', None],
                    'model__solver': ['lbfgs', 'saga', 'liblinear']
            ...
            "SVM": {
                'pipeline': create_pipeline(SVC(probability=True)),
                'params': {
                    'feature_selector': [PCA(n_components=0.95), 'passthrough'],
                    'model__C': [0.1, 1, 10],
                    'model__kernel': ['linear', 'rbf'],
                    'model__class_weight': ['balanced']
                }
            ...
            "XGBoost": {
                ...
                'params': {
                    ...
                    'model__learning_rate': [0.01, 0.1, 0.2],
                    'model__subsample': [0.8, 1.0],
                    'model__scale_pos_weight': [ratio]  # Балансировка для XGBoost
                }
            }
        }

        # 3. Обучение с групповой валидацией
        optimized_models = {}
        f2_scorer = make_scorer(fbeta_score, beta=2, pos_label=target_class)
        ...
        # 4. Исправленный стекинг с auto stack_method
        ensemble = StackingClassifier(
            estimators=[(name, model) for name, model in optimized_models.items()],
            final_estimator=LogisticRegression(max_iter=1000, class_weight='balanced'),  # Заменяем SVC на LogisticRegression
            stack_method='auto',  # Автоматический выбор метода
            n_jobs=-1
        )
        
        # 5. Корректная оценка ансамбля
        logo = LeaveOneGroupOut()
        f2_scores = []
        ...
        for train_idx, val_idx in logo.split(X, labels, groups=groups):
            ...
            # Сохранение F2-score
            f2 = fbeta_score(y_val, y_pred, beta=2, pos_label=target_class)
            f2_scores.append(f2)
        
        if verbose:
            print(f"Ансамбль - F2-score: {np.mean(f2_scores):.3f} ± {np.std(f2_scores):.3f}")
